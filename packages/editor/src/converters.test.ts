...
describe("proseMirrorToDocument", () => {
  it("round-trips through documentToProseMirror", () => {
      ...
      blocks: [
        ...
        listBlock({
          ...
          items: [
            {
              blocks: [
                paragraphBlock({ id: "b3-1", content: [{ text: "one" }] }),
              ],
            },
            {
              blocks: [
                paragraphBlock({ id: "b3-2", content: [{ text: "two" }] }),
              ],
            },
          ],
        }),
    ...
    expect(back.blocks.map((b) => b[0])).toEqual(
      original.blocks.map((b) => b[0]),
    );
  ...
  it("preserves custom block ids through a ProseMirror round trip", () => {
    ...
    expect(back.blocks[1]).toMatchObject(["Moonlight", { id: "diagram-2" }]);
  });

  it("normalizes stale or unknown Moonlight attrs into the public payload shape", () => {
    const back = proseMirrorToDocument(
      {
        type: "doc",
        content: [
...
describe("property-based converter invariants", () => {
  it("round-trips generated documents when ids are normalized away", () => {
    fc.assert(
      fc.property(
        papyrDocumentArbitrary({ maxBlocks: 4, maxDepth: 2 }),
        (original) => {
          const back = proseMirrorToDocument(
            documentToProseMirror(original),
            original.id,
          );
          expect(normalizeBlocks(back.blocks)).toEqual(
  ...
  it("preserves generated custom block ids through a ProseMirror round trip", () => {
    const customBlockArbitrary = fc.oneof(
      gfmCompatibleTableBlockArbitrary.map((block) =>
        tableBlock({ ...block[1], id: "table-custom" }),
      ),
      fencedTableBlockArbitrary.map((block) =>
        tableBlock({ ...block[1], id: "table-fenced" }),
      ),
      safeTextArbitrary.map((source) =>
        mermaidBlock({
          id: "mermaid-custom",
          source,
        }),
      ),
      safeTextArbitrary.map((caption) =>
        moonlightBlock({
          id: "moonlight-custom",
          svg: `<svg viewBox="0 0 120 80"><text>${caption}</text></svg>`,
        }),
      ),
    );
