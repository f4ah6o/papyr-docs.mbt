...
export type HeadingBlock = ['Heading', { id: string; level: number; content: Inline[] }];
export type ParagraphBlock = ['Paragraph', { id: string; content: Inline[] }];
export type ListBlock = ['List', { id: string; ordered: boolean; items: ListItem[] }];
export type CodeBlock = ['Code', { id: string; language?: string; source: string }];
export type TableBlock = [
  'Table',
  { id: string; columns: TableColumn[]; rows: TableCell[][]; caption?: string },
];
export type MermaidBlock = ['Mermaid', { id: string; source: string; caption?: string }];
...
export const InlineSchema = v.object({
  text: v.string(),
  marks: v.optional(v.array(v.picklist(['bold', 'italic', 'code', 'strike', 'link']))),
...
export const ParagraphBlockSchema = BlockSchema as v.GenericSchema<ParagraphBlock>;
...
export const ListItemSchema: v.GenericSchema<ListItem> = v.custom(
  (input): input is ListItem =>
    ...
    input.blocks.flatMap((block, index) => validateBlock(block, `blocks.${index}`)).length === 0,
  'Invalid PapyrDocument list item',
);

export type DocumentValidationPathSegment = string | number;
export type DocumentValidationIssues = [DocumentValidationIssue, ...DocumentValidationIssue[]];
...
export function getDocumentValidationIssues(
  issuesOrError: PapyrDocumentValidationError | AnyValiError | DocumentValidationIssues,
): DocumentValidationIssues {
  if (issuesOrError instanceof PapyrDocumentValidationError) return issuesOrError.issues;
  if (Array.isArray(issuesOrError)) return issuesOrError;
  return [
    issue(['document'], 'PapyrDocument', JSON.stringify(issuesOrError), 'Invalid PapyrDocument'),
  ];
}
...
export function formatDocumentValidationError(
  ...
  const renderedIssues = issues
    .slice(0, maxIssues)
    .map((validationIssue) => `- ${formatDocumentValidationIssue(validationIssue)}`);
  ...
  if (remainingIssues > 0) lines.push(`- ...and ${remainingIssues} more issue(s)`);
  return lines.join('\n');
}

export function formatDocumentValidationIssue(validationIssue: DocumentValidationIssue): string {
...
export function excalidrawBlock(payload: ExcalidrawBlock[1]): ExcalidrawBlock {
...
function validateDocumentShape(input: unknown): DocumentValidationIssue[] {
  if (!isObject(input)) return [issue([], 'PapyrDocument object', received(input))];
  const issues: DocumentValidationIssue[] = [];
  if (typeof input.id !== 'string') issues.push(issue(['id'], 'string', received(input.id)));
  else if (input.id.length === 0)
    issues.push(issue(['id'], 'non-empty string', 'empty string', 'id cannot be empty'));
...
function validateBlock(input: unknown, path: string): DocumentValidationIssue[] {
  ...
  if (!isObject(payload)) return [issue([...split(path), 1], 'object', received(payload))];
  const issues: DocumentValidationIssue[] = [];
  if (typeof payload.id !== 'string') issues.push(issue([...split(path), 'id'], 'string', received(payload.id)));
  switch (tag) {
    case 'Heading':
      if (typeof payload.level !== 'number' || payload.level < 1 || payload.level > 6) {
        issues.push(issue([...split(path), 'level'], 'integer in range 1..6', received(payload.level)));
      }
      issues.push(...validateInlineArray(payload.content, [...split(path), 'content']));
      break;
    case 'Paragraph':
      issues.push(...validateInlineArray(payload.content, [...split(path), 'content']));
      break;
    case 'List':
      if (typeof payload.ordered !== 'boolean')
        issues.push(issue([...split(path), 'ordered'], 'boolean', received(payload.ordered)));
      if (!Array.isArray(payload.items)) {
        issues.push(issue([...split(path), 'items'], 'Array', received(payload.items)));
      } else {
        payload.items.forEach((item, index) => {
          if (!isObject(item) || !Array.isArray(item.blocks) || item.blocks.length === 0) {
            issues.push(issue([...split(path), 'items', index, 'blocks'], 'non-empty Array', received(isObject(item) ? item.blocks : item)));
          } else {
            item.blocks.forEach((block, blockIndex) => {
              issues.push(...validateBlock(block, `${path}.items.${index}.blocks.${blockIndex}`));
            });
          }
        });
      }
    ...
    case 'Code':
      if (payload.language !== undefined && typeof payload.language !== 'string')
        issues.push(issue([...split(path), 'language'], 'string', received(payload.language)));
      if (typeof payload.source !== 'string')
        issues.push(issue([...split(path), 'source'], 'string', received(payload.source)));
      break;
    case 'Table':
    case 'Mermaid':
    case 'Excalidraw':
      break;
    default:
      issues.push(issue([...split(path), 0], '"Heading" | "Paragraph" | "List" | "Code" | "Table" | "Mermaid" | "Excalidraw"', JSON.stringify(tag)));
  }
  return issues;
}

function validateInlineArray(input: unknown, path: DocumentValidationPathSegment[]): DocumentValidationIssue[] {
  ...
  input.forEach((run, index) => {
    ...
    if (typeof run.text !== 'string') issues.push(issue([...path, index, 'text'], 'string', received(run.text)));
    if (run.marks !== undefined && !Array.isArray(run.marks)) {
      issues.push(issue([...path, index, 'marks'], 'Array', received(run.marks)));
    } else if (Array.isArray(run.marks)) {
      for (const [markIndex, mark] of run.marks.entries()) {
        if (!['bold', 'italic', 'code', 'strike', 'link'].includes(String(mark))) {
          issues.push(issue([...path, index, 'marks', markIndex], 'one of bold, italic, code, strike, link', received(mark)));
        }
      }
      if (run.marks.includes('link') && typeof run.href !== 'string') {
        issues.push(issue([...path, index, 'href'], 'string', received(run.href)));
      }
    }
  });
  return issues;
