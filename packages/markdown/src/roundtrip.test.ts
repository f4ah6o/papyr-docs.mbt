...
describe("parseMarkdown", () => {
  ...
  it("parses inline marks", () => {
    ...
    if (!isBlockKind(paragraph, "Paragraph"))
  ...
  it("decodes papyr-moonlight fences without preserving unknown keys", () => {
    const input = [
      "```papyr-moonlight",
      JSON.stringify({
        svg: '<svg viewBox="0 0 120 80"><text>Sketch</text></svg>',
        caption: "Sketch",
        elements: [{ text: "old" }],
  ...
  it("decodes mermaid fences", () => {
    ...
    expect(doc.blocks[0]).toEqual(
      mermaidBlock({ id: "b1", source: "graph TD; A-->B;" }),
    );
  });
  ...
  it("rejects images in table cells instead of silently dropping them", () => {
    ...
    expect(() => parseMarkdown(md)).toThrow(
      "Unsupported Markdown inline: image",
    );
  });
  ...
  it("rejects HTML in table cells instead of silently dropping it", () => {
    const md = "| Header |\n| ------ |\n| <span>html</span> |";
    expect(() => parseMarkdown(md)).toThrow(
      "Unsupported Markdown inline: html",
    );
  });
      ...
      blocks: [
        listBlock({
          ...
          items: [
            {
              blocks: [
                paragraphBlock({ id: "p1", content: [{ text: "one" }] }),
              ],
            },
            {
              blocks: [
                paragraphBlock({ id: "p2", content: [{ text: "two" }] }),
              ],
            },
          ],
        }),
      ...
      blocks: [
        listBlock({
          ...
          items: [
            ...
            {
              blocks: [
                paragraphBlock({ id: "p3", content: [{ text: "next" }] }),
              ],
            },
          ],
        }),
...
describe("parse/serialize round-trip", () => {
  it("round-trips a document with table, mermaid, paragraphs", () => {
    ...
    expect(reTable[1].rows).toEqual(
      isBlockKind(originalTable, "Table") ? originalTable[1].rows : [],
    );
  });
  ...
  it("round-trips arbitrary block sequences inside list items", () => {
    ...
    if (!isBlockKind(nestedList, "List"))
    ...
    fc.assert(
      fc.property(
        papyrDocumentArbitrary({
          maxBlocks: 4,
          maxDepth: 2,
          singleInlineRun: true,
        }),
        (original) => {
          const md = serializeDocument(original);
          const reparsed = parseMarkdown(md, { documentId: original.id });
          expect(normalizeBlocks(reparsed.blocks)).toEqual(
            normalizeBlocks(original.blocks),
          );
        },
      ),
      { numRuns: 40 },
  ...
  it("assigns deterministic unique keys when parsing generated GFM tables", () => {
    const gfmMarkdownArbitrary = fc
      ...
      .chain((columnCount) =>
        fc.record({
          ...
          row: fc.array(safeTextArbitrary, {
            minLength: columnCount,
            maxLength: columnCount,
          }),
        }),
    ...
    fc.assert(
      fc.property(gfmMarkdownArbitrary, ({ headers, row }) => {
        ...
        if (
          !isBlockKind(firstTable, "Table") ||
          !isBlockKind(secondTable, "Table")
        ) {
        ...
        expect(keys).toEqual(
          secondTable[1].columns.map((column) => column.key),
        );
      }),
      { numRuns: 40 },
    );
