...
export type EditableEmbeddedBlock = TableBlock | MermaidBlock | MoonlightBlock;
export type EditableEmbeddedBlockType = "table" | "mermaid" | "moonlight";

export function collectEmbeddedPreviewBlocks(
  doc: PapyrDocument,
): EditableEmbeddedBlock[] {
  const result: EditableEmbeddedBlock[] = [];
  collectBlocks(doc.blocks, result);
  return result;
}

export function getEmbeddedBlockEditorTitle(
...
export function embeddedBlockType(
  block: EditableEmbeddedBlock,
): EditableEmbeddedBlockType {
...
export function isEditableMoonlightBlock(
  block: EditableEmbeddedBlock | undefined,
): block is MoonlightBlock {
  return isBlockKind(block, "Moonlight");
}
