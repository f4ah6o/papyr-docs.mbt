...
slug: blocks
title: ブロックスキーマ
summary: 標準ブロックの一覧と、TableBlock / MoonlightBlock / MermaidBlock の扱い。
emoji: 🧩
published: true
topics: [blocks]
chapterOrder: 2
...
- `table` / `mermaid` / `moonlight` は「Markdown だけでは情報を落としやすいもの」を構造化した block
...
`table` と `moonlight` を JSON で保持するのも同じ理由です。見た目だけ Markdown に落とすのではなく、**再編集できる構造を残す** ことを優先しています。
