...
```json
{
  ...
  "blocks": [
    [
      "Heading",
      { "id": "h1", "level": 1, "content": [{ "text": "Hello Papyr" }] }
    ],
...
| `Moonlight` | `id`, `svg`, `caption`             | `papyr-moonlight` fence              |
    ...
    "items": [
      {
        "blocks": [
          [
            "Paragraph",
            { "id": "s1", "content": [{ "text": "Parse Markdown" }] }
          ]
        ]
      },
      {
        "blocks": [
          [
            "Paragraph",
            { "id": "s2", "content": [{ "text": "Render preview" }] }
          ]
        ]
      }
    ]
...
Validation は `id` が空でない文字列であること、`blocks` が配列であること、既知の block tag だけを含むこと、heading level や inline marks が契約内であることを確認します。Table、Mermaid、Moonlight は renderer や editor が扱う payload を保持するため、core では tag と object payload の境界を主に確認します。
