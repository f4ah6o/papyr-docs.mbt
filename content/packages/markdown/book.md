...
summary: Markdown と PapyrDocument の相互変換。papyr-table / papyr-excalidraw / mermaid フェンス対応。
emoji: 📝
published: true
topics: [markdown]
...
`papyr-table`、`papyr-excalidraw`、`mermaid` フェンスの変換を担当します。
...
Papyr 固有の table や Excalidraw payload は、`papyr-table` / `papyr-excalidraw` fence に JSON として保存します。GFM table で表現できる単純な table は Markdown table として round-trip できます。
