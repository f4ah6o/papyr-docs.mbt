...
3. table / Mermaid / Excalidraw の embedded preview と block editor を同じ workspace に載せる
...
`createSampleDocumentSource()` は Markdown・table・Mermaid・Excalidraw を含む serialized source を返します。
playground や story 的な画面で、空状態ではなく「Papyr らしい document」を最初から見せたいときに向いています。

## 依存関係

`react` / `react-dom` / `@excalidraw/excalidraw` は peer dependency です。UI package 自体は
