...
test("visual editor: embedded block previews are rendered inline", async ({
  page,
}) => {
  const tableBlock = page.locator(
    '.editor-workspace__prosemirror [data-papyr-block="table"]',
  );
  const mermaidBlock = page.locator(
    '.editor-workspace__prosemirror [data-papyr-block="mermaid"]',
  );
  const moonlightBlock = page.locator(
    '.editor-workspace__prosemirror [data-papyr-block="moonlight"]',
  );
  ...
  await expect(moonlightBlock.locator("svg")).toBeVisible();
});
...
test("visual editor: table caption の編集が block と markdown source に反映される", async ({
  ...
  await expect(tableBlock).toHaveAttribute(
    "data-papyr",
  ...
  await expect(tableBlock).not.toHaveAttribute(
    "data-papyr",
    /Publishing checklist/,
  );
...
test("visual editor: toolbar bold を適用すると ProseMirror content が強調される", async ({
  ...
  await page
    .locator(".editor-workspace__toolbar-button", { hasText: "Bold" })
    .click();

  await expect(
    editor.locator("strong").filter({ hasText: "bold-test" }),
  ).toBeVisible();
  await expect.poll(() => readStoredSource(page)).toContain("**bold-test**");
});

test("visual editor: bold selection では italic marker を誤って出力しない", async ({
  page,
}) => {
  await openPlayground(page, "bold-test");
  await expectVisualEditor(page);
  ...
  const editor = page.locator(".editor-workspace__prosemirror");
  const boldButton = page.locator(".editor-workspace__toolbar-button", {
    hasText: "Bold",
  });
  ...
  await expect.poll(() => readStoredSource(page)).toContain("**bold-test**");
  await expect
    .poll(() => readStoredSource(page))
    .not.toContain("*_bold-test_*");
});
...
test("mermaid: visual block を double click すると focused editor が開く", async ({
  page,
}) => {
  ...
  await expect(page.locator(".editor-workspace__modal")).toContainText(
    "Mermaid editor",
  );
  await expect(page.locator(".editor-workspace__mermaid-source")).toBeVisible();
  await expect(
    page.locator(".editor-workspace__modal-field input").first(),
  ).toBeFocused();
});
...
test("mermaid editor: source を更新すると markdown source も更新される", async ({
  page,
}) => {
  const mermaidBlock = page
    .locator('.editor-workspace__prosemirror [data-papyr-block="mermaid"]')
...
test("Add Moonlight で focused editor がすぐ開く", async ({ page }) => {
  const moonlightBlocks = page.locator(
    '.editor-workspace__prosemirror [data-papyr-block="moonlight"]',
  );
  const before = await moonlightBlocks.count();

  await page
    .locator("button.editor-workspace__action", { hasText: "Add Moonlight" })
    .click();

  await expect(moonlightBlocks).toHaveCount(before + 1);
  await expect(page.locator(".editor-workspace__modal")).toContainText(
    "Moonlight editor",
  );
  await expect(
    page.locator(".editor-workspace__moonlight--modal"),
  ).toBeVisible();
});
